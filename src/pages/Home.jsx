  ...
  return (
    <div>
      <div className="grid gridows-2 max-lg:grid-rws-6 grid-cols-5 max-lg:grid-cols-3 p-10 max-md:px-5 max-sm:px-0 border-b border-[#0a0a0a] max-sm:pb-5 pt-20">
        <div className="row-span-4 col-span-3 h-[500px] max-lg:h-ful p-2 max-lg:row-span-2 max-md:h-[300px]">
          <div className="flex justify-center items-start bg-[#333] relative w-full overflow-hidden  h-full rounded-lg flex-col con">
            ...
            <Link
              to={"/filteredProducts"}
              className="text-black transition-all duration-300 flex justify-start px-4 z-20 items-center font-bold w-20 h-10 rounded-md mx-10 bg-white hover:w-24 relative"
            >
              GO
              <BsArrowRight className="absolute right-3" />
            </Link>
            <img
              className=" h-full absolute right-0 top-0 -rotate-12 -translate-y-0 para  hover:rotate-0"
              src={Puma}
              alt=""
            />
          </div>
        ...
        <div className="row-span-1 max-lg:row-span-2 col-span-2 max-lg:col-span-3 p-2 h-[250px] max-lg:h-[300px] max-md:h-[250px]">
          <div className="bg- w-full h-full overflow-hidden rounded-lg bg-[#6B522E] relative con flex-col flex justify-center items-start">
            ...
            <img
              className="h-full absolute right-0 top-0 -rotate-12 -translate-y-0 para "
              src={Nike}
              alt=""
            />
          </div>
      ...
      <div className="exc uppercase flex flex-col justify-center items-center h-[400px] titletext leading-[44px] bg-[url('./assets/banner1.jpg')] bg-cover text-white">
        <span className="text-[24px] max-md:text-[18px] font-light">
          up to 30% off
        </span>
        <span className="text-[50px] max-md:leading-[24px] max-md:text-[32px]">
          Exclusive
        </span>
        <span className="text-[50px] max-md:text-[32px] font-bold">
          new shoes
        </span>
        <Link
          to={"/filteredProducts"}
          className="border-2 b px-5  m-5 hover:px-10 max-md:text-[12px]  transition-all duration-200 "
        >
          Shop Now
        </Link>
      </div>

      <div className="grid pt-20 grid-cols-3 max-lg:grid-cols-1 gap-20 px-20 max-lg:px-10 max-sm:px-5 max-md:pt-10 max-sm:pt-5">
        <div className="flex flex-col justify-center item relative overflow-hidden items-center border h-[250px] max-md:h-[400px] max-sm:h-[300px] col-span-1 text-white rounded">
          <img
            src={sales1}
            alt=""
            className="absolute -z-20 top-0 left-0 object-cover max-sm:h-[100%] lg:h-[100%] lg:w-full"
          />
      ...
      <div className="grid py-20 grid-cols-3 max-lg:grid-cols-1 gap-20 px-20 max-lg:px-10 max-sm:px-5 max-md:pt-10 max-sm:pt-5">
        <div className="flex flex-col justify-center item relative overflow-hidden items-center border h-[250px] max-md:h-[400px] max-sm:h-[300px] col-span-1 text-white rounded">
          <img
            src={sales4}
            alt=""
            className="absolute -z-20 top-0 left-0 object-cover max-sm:h-[100%] lg:w-full border"
          />
          <span className="text-[20px]">sale 50% off</span>
          <span className="font-bold text-[24px] max-lg:text-[40px] max-sm:text-[32px] uppercase">New Arrivals</span>
        </div>
